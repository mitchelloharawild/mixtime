...
#' 
#' @param x A time object (e.g., [base::Date], [base::POSIXct], [continuous_time()], etc.)
#' 
...
#' time_chronon(continuous_time(tu_month(1L), list(tu_year(1L)))(Sys.Date()))
time_chronon <- S7::new_generic("time_unit", "x")

S7::method(time_chronon, S7::new_S3_class("mt_continuous")) <- function(x) {
  attr(x, "chronon")
}
