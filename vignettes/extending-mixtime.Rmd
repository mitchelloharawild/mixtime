...
* Labels: 
  ...
  * Time labels: `linear_labels()`, `cyclical_labels()`
...
Another complication is that mixtime methods support n-unit time vectors (e.g. 2 months), so the divmod must also account for the possibility of converting from a multi-month period that may contain a leap week. If this generality is not needed, your method can raise an error when the time unit is not size 1 (e.g. `if (vctrs::vec_data(to) != 1L) stop("...")`).
...
S7::method(chronon_divmod, list(cal_symmetry454$week, cal_symmetry454$month)) <-
  function(from, to, x) {
    ...
    # Account for leap years which have a 293-year cycle (containing 52 leap weeks).
    ...
    x_cyc <- x + 11010L  # +1 for 0-indexing of x
    ...
    # Identify leap weeks: a week is a leap week if it is in the last regularised
    # position of a year AND a new leap week is introduced at that point.
    last_weeks  <- which(x_reg %% 52L >= 51L)
    leap_weeks  <- last_weeks[n_leaps[last_weeks] - leaps_symmetry454(x_cyc[last_weeks] - 1L) > 0L]
...
* `time_unit_abbr()` returns the short abbreviation of the unit (e.g. "S454Y")
...
S7::method(time_unit_abbr, cal_symmetry454$year)  <- function(x) "S454Y"
S7::method(time_unit_full, cal_symmetry454$month) <- function(x) "Symmetry454 month"
S7::method(time_unit_abbr, cal_symmetry454$month) <- function(x) "S454M"
S7::method(time_unit_full, cal_symmetry454$week)  <- function(x) "Symmetry454 week"
S7::method(time_unit_abbr, cal_symmetry454$week)  <- function(x) "S454W"
```
...
```{r}
cyclical_time(as.Date("1955-11-12"), chronon = cal_symmetry454$week(1L), cycle = cal_symmetry454$year(1L))
month_of_year(as.Date("1955-11-12"), calendar = cal_symmetry454)
```

While descriptive, the calendar scoping of "S454" in the time unit abbreviations produces format strings that are difficult to read. To improve readability, we can define time format methods that use more concise format strings.
...
With these linear and cyclical label methods in place, the default time formats now produce more informative labels for Symmetry454 time vectors:
...
Linear granule labels are evaluated with `linear_labels()`, and cyclical granule labels are evaluated with `cyclical_labels()`. The default methods for these generics return the internal numeric representation of the time point (e.g. "0", "1", "2", ...), but they can be overridden to return custom labels.
