...
It is common to need custom labels for cyclical time granules, since cyclical time points often have familiar labels (e.g. Jan, Feb, ... for months in years) or are 1-indexed (e.g. 1, 2, ... for days in a month). The default method for `cyclical_labels()` simply uses the internal 0-indexed numeric representation of the time point.

The position within the cycle is given by `i` and is 0-indexed (e.g. 0-11 for month of year). The linear position of the cycle itself is given by the `at` argument, which allows labels to vary between cycles. This is particularly useful for labelling irregular cycles (e.g. Hebrew months), however regular cycles (like Symmetry454 months) can ignore the `at` argument:
